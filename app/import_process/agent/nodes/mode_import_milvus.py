...
def step_2_prepare_collections(state):
    ...
    if not milvus_client.has_collection(collection_name=milvus_config.chunks_collection):
        ...
        schema.add_field(field_name="dense_vector", datatype=DataType.FLOAT_VECTOR, dim=embedding_config.dim)
...
def step_4_insert_collections(milvus_client, chunks):
    ...
    logger.info(f"完成数据插入，成功插入 {insert_count} 条数据")
...
def node_import_milvus(state: ImportGraphState) -> ImportGraphState:
    """
    节点: 导入向量库 (node_import_milvus) 将处理好的向量数据写入 milvus 数据库。
    """
    ...
    try:
        ...
        if not chunks:
            logger.error(f">>> [{function_name}] 缺少chunks数据，请检查")
            raise ValueError("缺少chunks数据")
