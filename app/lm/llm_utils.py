...
def get_llm_client(model: Optional[str] = None, json_mode: bool = False) -> ChatOpenAI:
    ...
    :param model: 模型名称，优先级：传入参数 > 配置文件 lm_config.llm_model
...
if __name__ == "__main__":
    ...
    try:
        ...
        # 测试2：指定对话模型 + 普通模式
        client2 = get_llm_client(model="GLM-5.2")
        logger.info("✅ 测试2通过：指定对话模型客户端创建成功")

        # 测试3：同一模型+模式，验证缓存命中
        client3 = get_llm_client(model="GLM-5.2")
        logger.info(f"✅ 测试3通过：缓存机制验证成功，client2与client3为同一实例：{client2 is client3}")

        # 测试4：开启JSON输出模式
        client4 = get_llm_client(model="GLM-5.2", json_mode=True)
